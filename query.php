...
HAVING COUNT(DISTINCT p.pid) = (SELECT COUNT(*) FROM Pezzi WHERE colore = 'rosso')";
    ...
    echo '<div class="sql-code">SELECT p.pnome FROM Pezzi p WHERE EXISTS (SELECT 1 FROM Catalogo c1 JOIN Fornitori f1 ON f1.fid = c1.fid WHERE c1.pid = p.pid AND f1.fnome = \'Acme\') AND NOT EXISTS (SELECT 1 FROM Catalogo c2 JOIN Fornitori f2 ON f2.fid = c2.fid WHERE c2.pid = p.pid AND f2.fnome <> \'Acme\');</div>';
